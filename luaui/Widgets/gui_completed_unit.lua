...
local minHeavyCost = 2000

-- case-sensitive. The widget will notify about these special cases
local specificUnits = {
    ["Tech Center Lvl1"] = true,
    ["Tech Center Lvl2"] = true,
    ["Tech Center Lvl3"] = true,
    ["Tech Center Lvl4"] = true,
    ["Advanced Solar Collector"] = true,
    ["Twilight"]=true,
    ["Exploiter"]=true,
    ["Advanced Fusion Reactor"]=true,
    ["Moho Mine"]=true,
}
...
local minAACost = minHeavyCost --500

-- enabling this means factories will be reported
local reportFactories = false --true

-- in seconds. Type "false" or "nil" (without the quotation marks) for infinite
local markerLifeSpan = 30
...
local SpSendMessageToAllyTeam = Spring.SendMessageToAllyTeam
...
local color_cyan = "\255\1\255\255"
...
function loadClasses()
  ...
  function Point:new(x, y, z, text, localOnly)
    ...
    if o.localOnly then
      ...
      local _,_,_,_,_,allyID = SpGetTeamInfo(myTeam)
      SpSendMessageToAllyTeam(allyID, o.text)
    end
