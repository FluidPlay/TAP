...
--[[
DOCUMENTATION:
 * This is unsynced only.
 * Use by calling WG.Tweener.InitTween (tweenData). Localize it, preferably.
]]
...
--    return math.atan2(math.sin(angle), math.cos(angle)) ---slower ver, below is faster (PS:less accurate, in tests)
...
--end
--
----Lerp functionality, may be used for turret aiming for instance
...
--        spEcho(" Couldn't find cmd or axis for piece: "..pieceID.." axis: "..axis)
--        return end
--
--    --local speedMult = 1.4
...
--        spEcho("Tween Error: Piece info couldn't be determined") -- for "..unitID)
--        return
--    end
...
--
--    --- Gotta normalize the current piece angle, since GetPieceTrans/Rot always uses it
--    --- (otherwise it results in non-shortest rotations)
...
--end
--
----- *** Internal piece tweening functionality ***
-----Recurses the tween until it's (fully) done, then returns to the caller (tweenPiece)
...
--        end
--    end
--    --Sleep(tweenData.sleepTime * 1000) --- Deprecated from springtweener.lua, just uses the global "sleepTime" above
...
--    tweenData.startGameFrame = currentFrame --spGetGameFrame()
