...
if gadgetHandler:IsSyncedCode() then
	...
	local teamsOnCARTS = {}			-- { teamID = allyTeamID, ... }
	...
	for _, playerID in ipairs(playerList) do
		...
		if not isSpec and isActive then
			...
			table.insert(allyTeams[allyTeamID], {teamID = teamID, playerID = playerID })
		end
	end

	--- TODO/WIP: if there's more than one shared teamID in the same allyTeam, tag it as a CARTS'ed team / allyTeam
	--- lowest playerID # will be the Architect, highest will be the Warlord
	local playerIDsOnTeam = {}
	for allyTeamID, data in pairs(allyTeams) do
		local teamIDs = #data
		for _, val in ipairs(data) do
			local teamID = val.teamID
			local playerID = val.playerID
			if not playerIDsOnTeam[teamID] then
				playerIDsOnTeam[teamID] = {}
			end
			table.insert(playerIDsOnTeam[teamID], playerID)
			if #playerIDsOnTeam[teamID] > 0 then
				teamsOnCARTS[teamID] = allyTeamID
			end
		end
	end
