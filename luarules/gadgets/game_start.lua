...
--include("luarules/libs/unit_spawner.lua")   --MaDD: TODO: Fix/add

local modOptions = Spring.GetModOptions()
local startUnits = { "bowhq", "kernhq",}
...
local function GetStartUnit(teamID)
    ...
    Spring.Echo("Start unit from GetSideData: "..(Spring.GetSideData(side) or "nil"))
...
local function SpawnStartUnit(teamID)
    ...
    Spring.Echo("Start Unit: "..(startUnit or "nil"))
    --local factionStr = ''
    local wasRandom = false
    if startUnit == "random" then
        --factionStr = 'random_'
        local idx = math.random(1,#startUnits)
        startUnit = startUnits[idx]
        wasRandom = true
    end

    --factionStr = factionStr..string.sub(startUnit,0,string.find(startUnit,"_")-1)
    Spring.SetTeamRulesParam(teamID, 'faction', startingSide , {public=true})
    if wasRandom then
        Spring.SendMessageToTeam( teamID, "Random faction assigned : "..unitToFaction(startUnit))
                --string.upper(string.sub(startUnit,0,string.find(startUnit,"_")-1) ) )
    end
    ...
    if (startUnit and startUnit ~= "") then
        ...
        if (GG.mFAIStartPosByTeamId and GG.mFAIStartPosByTeamId[teamID]) then
...
function gadget:GameFrame(f)
    if (f == 1) then
        for uId,_ in pairs(idsRequiringTeleportFx) do
            ---TODO: Add teleport-in UnitFX (that's from MetalFactions.SDD) //MaDD
