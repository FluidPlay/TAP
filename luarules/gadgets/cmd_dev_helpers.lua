...
----------------------------------------
    ...
    function GiveCat(_,line, words, playerID)
        ...
        -- factions
        if string.find(line, "bow") then
            local Condition = function (ud) return ud.name:sub(1,3)=="bow" end
            Accept[#Accept+1] = Condition
        end
        if string.find(line, "kern") then
            local Condition  = function (ud) return ud.name:sub(1,3)=="kern" end
            Accept[#Accept+1] = Condition
        end
